...
### 노트북 덮으면, 외장 드라이브는 알아서 빠집니다.

매번 "안전하게 제거" 클릭하는 일도, *"디스크가 정상적으로 추출되지 않았습니다"* 알림도 — 이제 그만.
...
### 😱 아침에 노트북 깨우니 — "디스크가 정상적으로 추출되지 않았습니다" 알림이 화면 가득

밤사이 잠자기가 풀렸다 잠들었다 반복하며 외장이 떨어졌다 붙었다.
아침에 노트북 열면 **알림이 10개, 20개씩** 쌓여 있음. X 누르며 비우는 데만 10분.

> *"엄청 찝찝하고, 하드 망가질 것 같은 두려움까지 생깁니다."* — 한 사용자

### 😮‍💨 "외장 안 빼고 가방에 넣어버렸다"

노트북 덮고 가방으로. 다음에 열어 보니 외장은 그동안 도킹 분리됐던 상태. **"디스크가 정상적으로 추출되지 않았습니다"** 알림. 작업 중이던 파일, 무사한지 한참 걱정.

### 🔄 Time Machine 백업이 또 끊김

작업 중에 외장이 그냥 끊김 → 백업 다시 처음부터.
다음 백업까지 또 1~2시간. 그동안 노트북은 가방 안.

### 😮‍💨 매번 클릭, 매번 클릭, 매번 클릭

작업 끝낼 때마다 — 메뉴바 → 외장 이름 → "추출". USB 3개면 3번. 하루 5번, 한 달 150번.
이 동작에만 쓰는 시간, 모아두면 꽤 됩니다.

---

**DiskOUT** 는 위 네 가지를 한 번에 해결하는, 메뉴바에서만 동작하는 작은 유틸리티입니다.

---

## 무엇을 해결하나요?

### 1️⃣ 잠자기 = 자동 추출 (이 앱의 진짜 가치)

DiskOUT 의 핵심. **노트북을 덮는 순간, 모든 외장이 안전하게 추출**됩니다.

- 가방에 넣기 전 메뉴바 클릭? 필요 없음.
- 다음에 노트북 열면 — 자동으로 다시 마운트, 작업 그대로 이어서.
- *"디스크가 정상적으로 추출되지 않았습니다"* 알림? 더 이상 안 봅니다.

### 2️⃣ 5초 → 1초 — <kbd>⌥</kbd><kbd>⌘</kbd><kbd>E</kbd>

USB 3개 + SD 카드 + 백업 외장. 일일이 메뉴 열고 클릭하지 마세요.

- **<kbd>⌥</kbd><kbd>⌘</kbd><kbd>E</kbd> 한 키 = 전체 추출** (어디서든, 다른 앱 쓰고 있어도)
- 키보드에서 손 안 떼고
- 또는 메뉴바 아이콘 **우클릭** = 즉시 전체 추출

### 3️⃣ 지금 몇 개? — 메뉴바에 숫자로

⏏ 같은 정적 아이콘 대신 **숫자**. `1`, `2`, `3` …

- 한눈에 연결 개수 확인 — 꽂힌 줄도 모르고 노트북 덮을 일 없음
- RAID · 멀티 파티션 · APFS 합성 볼륨도 "한 장치" 기준 1개로 합산
- 추출 진행 / 결과는 ↻ · ✓ · ✗ 로 잠깐 표시
