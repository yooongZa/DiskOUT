...
| **쓰는 중 디스크 표시** | 외장에 쓰기 I/O 가 진행 중이면 메뉴바 숫자 옆 작은 systemBlue `●` + "쓰는 중 — 분리하지 마세요" tooltip (업데이트 알림의 빨간 `●` 와 색으로 구분). 메뉴에서는 **쓰는 중인 그 디스크 항목 옆에만** 파란 `●` — 어떤 디스크가 바쁜지 바로 식별. 물리 디스크 I/O 카운터(IORegistry)를 1.5s 폴링, 볼륨→물리 매핑은 parent-walk 로 RAID·APFS 합성·직결 모두 처리. 외장 있을 때만 가동 (배터리) |
| **디스크 용량 / 사용률** | 각 디스크 메뉴 항목 2번째 줄에 *남은 용량 / 전체 용량 (사용률 %)* 표시 — 예 `2.9 TB free of 7.3 TB (40% used)`. 메뉴 열 때 `URLResourceValues` 로 조회 (프로세스 호출 없음) |
| 개별 추출 | 드라이브 이름 클릭 |
| **쓰는 중 추출 확인** | 쓰는 중인 디스크를 **수동** 추출하면 확인 대화상자 (force fallback 이 복사를 끊어 파일 손상시키는 사고 방지). "그래도 추출" 선택 시에만 진행. sleep · 화면 꺼짐 · 자동 경로는 확인 없이 기존 force 동작 |
