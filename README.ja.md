...
ディスク操作、数字表示、基本キャラクター13種類は無料です。有料なのは季節の特別キャラクターパックのみ。2026ハロウィーン10種類はUSD 3.90の買い切りで、一年中使えます。設定 → キャラクターのドロップダウンで選び、10種類の動きをプレビューできます。
...
| **メニューバーアイコン = キャラクター/⏏ + マウント数** | 数字表示と基本キャラクター13種類は無料。キャラクターはドライブの活動に反応し、ドライブ2台に対応する自転車は活動に応じて休息・移動・速い動きを切り替え、休息中はZzzが浮かぶ。ハロウィンは実際の0〜9台に対応する形へ自動切り替え。10台以上は正確な数字を表示。基本セットも13台以上は数字になる。取り出しの進行・結果表示が優先され、動きを減らす設定とスリープ中はアニメーションを停止。 |
| **基本キャラクター13種類** | 0〜12台に対応し、顔と体の比率を統一。2は普通の自転車、3は先端がとがった三本足のタコ、8は八本足のタコ、9は九尾のキツネ。通常は1フレーム120ms、高負荷時は80ms。自転車は8フレーム、その他は6フレームで動き、待機中は眠る動作に移行。 |
| **ハロウィン数字キャラクター0〜9** | 0 カボチャ · 1 数字の1の形の杖 · 2 ハロウィン自転車 · 3 ケルベロス · 4 四本足で歩く机 · 5 魔女帽子をかぶった星 · 6 六方向のクモの巣 · 7 数字の7の形の鎌 · 8 八本足のクモ · 9 九尾のキツネ。実際のドライブ数に応じて自動で切り替わり、10台以上は数字で表示。 |
...
| **環境設定ウィンドウ** | <kbd>⌘</kbd><kbd>,</kbd> またはメニューの「設定…」。システム設定式の**ツールバー 6 ペイン** — 一般 (ログイン · 言語 · エラー報告) / 取り出しの動作 (sleep · display sleep · Music/Photos · 強制アンマウント · 右クリック) / 通知 / ホットキー / Characters (購入 · 復元 · 状態) / About (バージョン · アップデート · リンク)。ペインごとに高さを自動調整し、分かりにくいオプションには説明行を表示 |
...
| **多言語 (ko + en + ja + zh-Hans)** | `Localizable.xcstrings` 204 個のキー。システムの優先言語リスト全体から最初の対応言語を選び、該当がない場合のみ英語にフォールバック。設定 → 一般 → Language でシステム設定または明示言語を選択可能 |
...
├── Localizable.xcstrings        # ko + en + ja + zh-Hans 翻訳 (Xcode String Catalog、204 キー)
