...
毎回「安全に取り出す」をクリックする手間も、*「ディスクが正しく取り出されませんでした」*通知も — もう終わり。
...
### 😱 朝 Mac を目覚めさせると —「ディスクが正しく取り出されませんでした」通知が画面いっぱい

夜のスリープ中に外付けが何度も外れて — 朝、通知が **10 個も 20 個も** 積もる。X ボタンを 1 つずつ閉じるのに 10 分。

> *「すごく気持ち悪くて、ハードが壊れるんじゃないかという不安まで湧いてきます」* — あるユーザー

### 😮‍💨 「取り出さずに鞄に入れちゃった」

ノートを閉じて鞄へ。次に開いたら外付けはドック分離されていた状態。**「ディスクが正しく取り出されませんでした」** 通知。作業中だったファイル、無事かどうか一晩中心配。

### 🔄 Time Machine バックアップが、また途中で切れた

作業中に外付けが切断 → バックアップやり直し。次のバックアップまでまた 1 ~ 2 時間。その間、ノートは鞄の中。

### 😮‍💨 毎回クリック、毎回クリック、毎回クリック

作業終了のたびにメニューバー → 外付け名 → 「取り出し」。USB 3 つなら 3 回。1 日 5 回、月 150 回。
この動作に使う時間、積もれば結構なものに。

---

**DiskOUT** は上記の 4 つを一度に解決する、メニューバーだけで動く小さなユーティリティです。

---

## 何を解決するか

### 1️⃣ スリープ = 自動取り出し(このアプリの本当の価値)

DiskOUT の核心。**ノートを閉じる瞬間、すべての外付けが安全に取り出されます。**

- 鞄に入れる前のメニューバークリック? 必要なし。
- 次に Mac を開けば → 自動で再マウント、作業はそのまま続き。
- *「ディスクが正しく取り出されませんでした」*通知? もう見なくなります。

### 2️⃣ 5 秒 → 1 秒 — <kbd>⌥</kbd><kbd>⌘</kbd><kbd>E</kbd>

USB 3 つ + SD カード + バックアップ外付け。いちいちメニュー開いてクリックしないでください。

- **<kbd>⌥</kbd><kbd>⌘</kbd><kbd>E</kbd> 1 つで全取り出し**(どこでも、他のアプリ作業中でも)
- キーボードから手を離さず
- またはメニューバーアイコン**右クリック** = 即座に全取り出し

### 3️⃣ 今、何個? — メニューバーに数字で

⏏ のような固定アイコンの代わりに**数字**。`1`、`2`、`3` …

- 一目で接続数を確認 — 挿してることを忘れて蓋を閉じる心配なし
- RAID · マルチパーティション · APFS 合成ボリュームも「1 デバイス」として集計
- 取り出し進行 / 結果は ↻ · ✓ · ✗ で短時間表示
