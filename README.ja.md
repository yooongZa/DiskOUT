...
ディスク操作、数字表示、基本キャラクター13種類と基本のループアニメーションは無料です。基本プレミアムモーションパックはUSD 1.90、ハロウィーン10種類＋モーションパックはUSD 3.90の独立した買い切りです。設定 → キャラクターでプレビューして購入できます。
...
| **メニューバーアイコン = キャラクター/⏏ + マウント数** | 数字表示、基本13種類と基本のループは無料。基本モーションパックはドライブの活動に反応し、ドライブ2台に対応する自転車は活動に応じて休息・移動・速い動きを切り替え、休息中はZzzが浮かぶ。ハロウィンは実際の0〜9台に対応する形へ自動切り替え。10台以上は正確な数字を表示。基本セットも13台以上は数字になる。取り出しの進行・結果表示が優先され、動きを減らす設定とスリープ中はアニメーションを停止。 |
| **基本キャラクター13種類** | 0〜12台に対応し、顔と体の比率を統一。2は普通の自転車、3は先端がとがった三本足のタコ、8は八本足のタコ、9は九尾のキツネ。無料の基本ループは6フレーム・1フレーム140msを維持し、新しい原画で外見と動きを調整。 |
...
| **多言語 (ko + en + ja + zh-Hans)** | `Localizable.xcstrings` 218 個のキー。システムの優先言語リスト全体から最初の対応言語を選び、該当がない場合のみ英語にフォールバック。設定 → 一般 → Language でシステム設定または明示言語を選択可能 |
...
├── Localizable.xcstrings        # ko + en + ja + zh-Hans 翻訳 (Xcode String Catalog、218 キー)
