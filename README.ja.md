...
| **環境設定ウィンドウ** | <kbd>⌘</kbd><kbd>,</kbd> またはメニューの「設定…」。システム設定式の**ツールバー 5 ペイン** — 一般 (ログイン · 言語) / 取り出しの動作 (sleep · display sleep · Music/Photos · force fallback · 右クリック) / 通知 / ホットキー / About (バージョン · アップデート確認 · リンク)。ペインごとに高さ自動切り替え、分かりにくいオプションには説明行付き |
