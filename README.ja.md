...
### メニューバーから外付けドライブをワンクリックで安全に取り出し

macOS 用の小さくて素早いメニューバーユーティリティ。

**한국어 · English · 日本語 · 中文 (简体)** · スリープ取り出し · Apple Silicon ネイティブ
...
> Mac に **USB · SSD · SD カード · 外付け RAID** などの外部ディスクをよく挿す方。<br>
> ノートを鞄に入れる前に毎回「安全に取り出す」をクリックするのが面倒な方。<br>
> ドック取り外し / 「取り出し失敗 (improperly ejected)」通知に一度でも痛い目にあった方。

DiskOUT はそんな方のために作った、小さくて素早いメニューバーユーティリティです。

---

## 3 つの中核機能

### ワンクリック、またはショートカット 1 回

メニューバーアイコンをクリック → 外付けリスト → 名前をクリックで即座に安全取り出し。
キーボードに手があるなら <kbd>⌥</kbd><kbd>⌘</kbd><kbd>E</kbd> で一括取り出し。

### スリープ = 自動取り出し

ノートを閉じれば外付けは勝手に外れます。鞄に入れる前に 1 つずつクリックする必要なし。
復帰すると自動で再マウント — ユーザーは**意識せず**外付け保護を受けます。

### メニューバーに外付け数を表示

固定の ⏏ アイコンの代わりに**数字**で表示。今何個挿さっているか一目で確認。
RAID · マルチパーティション · APFS 合成ボリュームも「挿したデバイス数」基準で 1 個に集計。
