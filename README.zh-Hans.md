...
| **写入中磁盘提示** | 当外置磁盘有写入 I/O 进行时,菜单栏数字旁出现小的 systemBlue `●` +「正在写入 — 请勿断开」tooltip (与更新通知的红色 `●` 以颜色区分)。在菜单中,蓝色 `●` **仅出现在正在写入的那个磁盘项旁** — 一眼识别哪个磁盘繁忙。轮询物理磁盘 I/O 计数器 (IORegistry),间隔 1.5s;卷→物理映射通过 parent-walk 统一处理 RAID · APFS 合成 · 直连。仅在有外置磁盘时运行 (省电) |
| **磁盘容量 / 使用率** | 每个磁盘的菜单项第二行显示*可用 / 总容量 (已用 NN%)* — 例如 `2.9 TB free of 7.3 TB (40% used)`。打开菜单时通过 `URLResourceValues` 读取 (无进程调用) |
| 个别弹出 | 点击驱动器名称 |
