...
### 😱 早上唤醒 Mac → *"未正确推出磁盘"* 通知铺满屏幕

夜里休眠时外置反复脱离 → 早上一打开,通知堆了 **10 个、20 个**。一个一个点 X 关掉,光这事就 10 分钟。

> *"特别膈应,担心硬盘会不会坏掉。"* — 一位用户

### 😮‍💨 "忘记拔了就装进包里"

合上盖子放进包,下次打开外置一直处于断开状态。**"未正确推出磁盘"** 通知。整晚担心数据是否安全。

### 🔄 Time Machine 备份又中断了

工作时外置突然断开 → 备份从头开始。下次备份又要等 1~2 小时。这段时间笔记本还在包里。

### 😮‍💨 每次点击,每次点击,每次点击

每次工作结束 — 菜单栏 → 外置名称 → "推出"。3 个 USB 就 3 次。一天 5 次,一个月 150 次。
这个动作累计起来,时间不少。

---

**DiskOUT** 就是为一次性解决这 4 件事而做的、仅在菜单栏中工作的小工具。
...
DiskOUT 的核心。**笔记本进入睡眠的瞬间,所有外置安全推出。**
...
3 个 USB + SD 卡 + 备份外置。别再一个个打开菜单点了。

- **<kbd>⌥</kbd><kbd>⌘</kbd><kbd>E</kbd> 一键全部推出**(任何位置,即使在其他应用中)
- 手不离键盘
- 或菜单栏图标 **右键** = 立即全部推出

### 3️⃣ 现在几个? — 菜单栏直接显示数字

不再是 ⏏ 静态图标,而是**数字**。`1`、`2`、`3` …

- 一眼看到连接数 — 不会忘了还插着就合盖子
- RAID · 多分区 · APFS 合成卷都按"一个设备"计 1
- 推出进行 / 结果用 ↻ · ✓ · ✗ 短暂显示
