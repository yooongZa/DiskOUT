...
**한국어 · English · 日本語 · 中文 (简体)** · 睡眠时弹出 · Apple Silicon 原生
...
> 经常往 Mac 上插 **USB · SSD · SD 卡 · 外置 RAID** 等外部磁盘的人。<br>
> 觉得每次把笔记本放进包之前都要点"安全推出"很麻烦的人。<br>
> 曾经因为底座断开 / "未正确推出磁盘 (improperly ejected)" 警告而吃过亏的人。

DiskOUT 就是为这些场景做的小巧快速的菜单栏工具。
...
点菜单栏图标 → 外置列表 → 点名称即可立即安全弹出。
手在键盘上的话,<kbd>⌥</kbd><kbd>⌘</kbd><kbd>E</kbd> 一次性全部弹出。

### 睡眠 = 自动弹出

合上盖子,外置设备自动脱开。塞进包之前不用一个个点击。
唤醒时自动重新挂载 — 用户**无感**享受外置保护。

### 菜单栏图标显示外置数量

不再是固定的 ⏏ 图标,而是**数字**显示。一眼就能看到现在插了几个。
RAID · 多分区 · APFS 合成卷都按"已插设备数"算作 1 个。
