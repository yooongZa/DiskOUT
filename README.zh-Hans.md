...
磁盘操作、数字显示、13款基础角色及基础循环动画均免费。基础角色高级动作包为USD 1.90，万圣节10款角色＋动作包为USD 3.90，可独立一次性购买。可在设置 → 角色中预览和购买。
...
| **菜单栏图标 = 角色/⏏ + 挂载数量** | 数字显示、13款基础角色及基础循环动画均免费。基础动作包随磁盘活动变化，代表2块磁盘的自行车随活动切换休息、移动和更快的移动，休息时会飘出Zzz。万圣节角色按实际0～9块磁盘自动匹配数字含义；10块及以上显示准确数字。基础套装超过12块也使用数字。推出进度和结果优先显示；减弱动态效果和睡眠期间停止动画。 |
| **13款基础角色** | 对应0～12块磁盘，统一脸部设计和身体比例。2是普通自行车，3是三条末端尖细触腕的章鱼，8是八腕章鱼，9是九尾狐。免费基础循环保留6帧、每帧140ms，使用新原画调整外观和动作。 |
...
| **多语言 (ko + en + ja + zh-Hans)** | `Localizable.xcstrings` 218 个 key。遍历完整的系统首选语言列表并选择第一个受支持语言，仅在全部不受支持时回退到英语。可在设置 → 通用 → Language 中选择跟随系统或明确指定语言 |
...
├── Localizable.xcstrings        # ko + en + ja + zh-Hans 翻译 (Xcode String Catalog,218 个 key)
