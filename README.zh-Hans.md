...
磁盘操作、数字显示、13款基础角色均免费。仅季节特别角色包收费：2026万圣节含10款角色，USD 3.90一次性购买，全年可用。在设置 → 角色的下拉菜单中选择，并预览10款角色的动作。
...
| **菜单栏图标 = 角色/⏏ + 挂载数量** | 数字显示和13款基础角色均免费，角色会随磁盘活动自动变化，代表2块磁盘的自行车随活动切换休息、移动和更快的移动，休息时会飘出Zzz。万圣节角色按实际0～9块磁盘自动匹配数字含义；10块及以上显示准确数字。基础套装超过12块也使用数字。推出进度和结果优先显示；减弱动态效果和睡眠期间停止动画。 |
| **13款基础角色** | 对应0～12块磁盘，统一脸部设计和身体比例。2是普通自行车，3是三条末端尖细触腕的章鱼，8是八腕章鱼，9是九尾狐。普通活动每帧120ms，高活动每帧80ms。自行车使用8帧，其他角色使用6帧，空闲时进入睡眠动作。 |
| **万圣节数字角色0～9** | 0 南瓜 · 1 数字1形魔杖 · 2 万圣节自行车 · 3 刻耳柏洛斯 · 4 用四条腿走路的书桌 · 5 戴女巫帽的星星 · 6 六向蛛网 · 7 数字7形镰刀 · 8 八足蜘蛛 · 9 九尾狐。根据实际磁盘数量自动切换角色，10块及以上显示为数字。 |
...
| **设置窗口** | <kbd>⌘</kbd><kbd>,</kbd> 或菜单“设置…”。采用系统设置风格的**6 个工具栏面板** — 通用 (登录 · 语言 · 错误报告) / 推出行为 (sleep · display sleep · Music/Photos · 强制卸载 · 右键) / 通知 / 热键 / Characters (购买 · 恢复 · 状态) / About (版本 · 更新 · 链接)。窗口高度随面板自动调整，不直观的选项均带说明行 |
...
| **多语言 (ko + en + ja + zh-Hans)** | `Localizable.xcstrings` 204 个 key。遍历完整的系统首选语言列表并选择第一个受支持语言，仅在全部不受支持时回退到英语。可在设置 → 通用 → Language 中选择跟随系统或明确指定语言 |
...
├── Localizable.xcstrings        # ko + en + ja + zh-Hans 翻译 (Xcode String Catalog,204 个 key)
