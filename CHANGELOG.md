...
## 2026-05-29: v0.4.4 — 외장 쓰기 활동 표시 (분리 경고) + 디스크 용량·사용률 메뉴 표기
...
```
[로고] [언어 스위처] [Hero: Mac 필수 무료 프로그램, DiskOUT 을 소개합니다]
   ↓
[배지 + 다운로드 버튼]
   ↓
...
Apple 의 SSD 가격은 미쳤습니다... 외장하드로 버티고 있지만... DiskOUT 을 사용하면...
   ↓
## 어떻게요?
① 노트북을 덮는 순간, 모든 외장이 안전하게 추출됩니다.
② 10개의 외장 하드도 한 번에 추출하세요.
③ 외장 디스크가 몇 개인지, 한눈에 확인하세요.
```

### 핵심 변화

- **헤딩 = SNS / 블로그 톤** ("Mac 필수 무료 프로그램, DiskOUT 을 소개합니다")
- **lead 줄** "Mac 은 완벽합니다 — *그 알림* 만 빼면" 으로 dry humor + Apple 비판 동시 점유
- **사용자가 외장을 쓰는 *이유* 호명** ("Apple 의 SSD 가격은 미쳤습니다") — 시장 조사에서 한국 + 미국 사용자 양쪽 모두 공감하던 페인포인트의 *원인* 까지 짚음
- **핵심 3가지 압축** — 각 항목 2~3 줄로 단순화 (이전 5+줄 bullet 구조 제거)
- **"보통의 어제 / 오늘" Before/After 영역 제거** — 더 직설적 흐름

### 4 언어 동일 mirror

각 언어의 macOS 알림 ("디스크가 정상적으로 추출되지 않았습니다" / "Disk Not Ejected Properly" / 「ディスクが正しく取り出されませんでした」/ "未正确推出磁盘") + "Apple SSD 가격" 비판은 4 시장 모두 통하는 페인포인트로 그대로 mirror.

### 변경 파일

- `README.md` / `README.en.md` / `README.ja.md` / `README.zh-Hans.md` — Hero + Section 2-3 통째 교체. 그 뒤 영역 유지.
